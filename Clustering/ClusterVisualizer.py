...
class ClusterVisualizer:
    ...
    def plot_data(self, title='Data Points'):
        ...
        '''
        fig = px.scatter(x=self.data.iloc[:, 0], y=self.data.iloc[:, 1])
        fig.update_traces(marker=dict(size=8, opacity=0.6))
        fig.update_layout(coloraxis_showscale=False, showlegend=False,
                          xaxis_title=self.data.columns[0], yaxis_title=self.data.columns[1],
    ...
    def plot_data_with_true_labels(self, title='Data Points with True Labels'):
        ...
        fig = px.scatter(x=self.data.iloc[:, 0], y=self.data.iloc[:, 1],
                         color=self.true_labels.astype(str), color_continuous_scale='jet')
        fig.update_traces(marker=dict(size=8, opacity=0.6))
        fig.update_layout(showlegend=True, legend_title_text='True Labels',
    ...
    def plot_clusters(self, labels=None, centers=None, title=''):
        ...
        fig = px.scatter(x=self.data.iloc[:, 0], y=self.data.iloc[:, 1], 
                         color=labels.astype(str), color_continuous_scale='portland')
        fig.update_traces(marker=dict(size=8, opacity=0.6))        
    ...
    def plot_silhouette(self, labels, title='Silhouette Scores'):
        ...
        '''
        silh_scores = np.array([silhouette_score(self.data, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = silh_scores.argmax()  
        colors = [0] * len(silh_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=silh_scores, text=silh_scores,                      
                     color=colors, color_continuous_scale='portland', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
    ...
    def plot_davies_bouldin(self, labels, title='Davies-Bouldin Scores'):
        ...
        labels: pandas DataFrame with the cluster assignments for each point in data.
        '''
        db_scores = np.array([davies_bouldin_score(self.data, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = db_scores.argmin()  
        colors = [0] * len(db_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=db_scores, text=db_scores,                      
                     color=colors, color_continuous_scale='portland', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
                          xaxis_title='Cluster', yaxis_title='Davies-Bouldin Score',
                          coloraxis_showscale=False, showlegend=False)
        fig.update_traces(texttemplate='%{text:.2f}', textposition='outside')
    ...
    def plot_calinski_harabasz(self, labels, title='Calinski-Harabasz Scores'):
        ...
        labels: pandas DataFrame with the cluster assignments for each point in data.
        '''
        ch_scores = np.array([calinski_harabasz_score(self.data, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = ch_scores.argmax()  
        colors = [0] * len(ch_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=ch_scores, text=ch_scores,                      
                     color=colors, color_continuous_scale='portland', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
                          xaxis_title='Cluster', yaxis_title='Calinski-Harabasz Score',
                          coloraxis_showscale=False, showlegend=False)
        fig.update_traces(texttemplate='%{text:.2f}', textposition='outside')
    ...
    def plot_adjusted_rand(self, labels, title='Adjusted Rand Scores'):
        ...
        if self.true_labels is None:
            raise ValueError('True labels are not provided.')
        ar_scores = np.array([adjusted_rand_score(self.true_labels, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = ar_scores.argmax()  
        colors = [0] * len(ar_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=ar_scores, text=ar_scores,                      
                     color=colors, color_continuous_scale='geyser', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
                          xaxis_title='Cluster', yaxis_title='Adjusted Rand Score',
                          coloraxis_showscale=False, showlegend=False)
    ...
    def plot_adjusted_mutual_info(self, labels, title='Adjusted Mutual Info Scores'):
        ...
        if self.true_labels is None:
            raise ValueError('True labels are not provided.')
        am_scores = np.array([adjusted_mutual_info_score(self.true_labels, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = am_scores.argmax()  
        colors = [0] * len(am_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=am_scores, text=am_scores,                      
                     color=colors, color_continuous_scale='geyser', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
                          xaxis_title='Cluster', yaxis_title='Adjusted Mutual Info Score',
                          coloraxis_showscale=False, showlegend=False)
        fig.update_traces(texttemplate='%{text:.3f}', textposition='outside')
    ...
    def plot_normalized_mutual_info(self, labels, title='Normalized Mutual Info Scores'):
        ...
        if self.true_labels is None:
            raise ValueError('True labels are not provided.')
        nm_scores = np.array([normalized_mutual_info_score(self.true_labels, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = nm_scores.argmax()  
        colors = [0] * len(nm_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=nm_scores, text=nm_scores,                      
                     color=colors, color_continuous_scale='geyser', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
                          xaxis_title='Cluster', yaxis_title='Normalized Mutual Info Score',
                          coloraxis_showscale=False, showlegend=False)
        fig.update_traces(texttemplate='%{text:.3f}', textposition='outside')
    ...
    def plot_fowlkes_mallows(self, labels, title='Fowlkes Mallows Scores'):
        ...
        if self.true_labels is None:
            raise ValueError('True labels are not provided.')
        fm_scores = np.array([fowlkes_mallows_score(self.true_labels, labels[col]) 
                                for col in labels.columns]).round(4) 
        best_score = fm_scores.argmax()  
        colors = [0] * len(fm_scores)
        colors[best_score] = 1        
        fig = px.bar(x=labels.columns, y=fm_scores, text=fm_scores,                      
                     color=colors, color_continuous_scale='geyser', opacity=0.7)
        fig.update_layout(width=self.width, height=self.height, title=title,
                          xaxis_title='Cluster', yaxis_title='Fowlkes Mallows Score',
                          coloraxis_showscale=False, showlegend=False)
        fig.update_traces(texttemplate='%{text:.3f}', textposition='outside')
    ...
    def plot_DBSCAN_categories(self, labels, core_sample_indices=[], title=''):
        ...
        symbol_map = {'Noise': 'circle', 'Core': 'circle', 'Border': 'circle-open'}
        
        # Plot the scatter with colors mapped to clusters
        fig = px.scatter(x=self.data.iloc[:,0], y=self.data.iloc[:,1], 
                         symbol=point_type, symbol_map=symbol_map,     
    ...
    def plot_optics_dbscan(self, optics, eps=0.05):
        ...
        fig = px.scatter(x=self.data.iloc[:,0], y=self.data.iloc[:,1], 
                         color=labels, color_discrete_map=color_map)
        fig.update_layout(coloraxis_showscale=False, 
                          width=self.width, height=350,
                          title='OPTICS Clustering (eps = %.2f)' % eps,
        ...
        for cluster in unique_clusters:
            ...
            fig.add_trace(go.Scatter(x=space[cluster_indices], 
                            y=optics.reachability_[optics.ordering_][cluster_indices],
                            mode='markers', marker_size=5, marker_color=color_map[cluster],    
                            name=cluster))
        fig.update_layout(width=self.width, height=350, 
                        title='Reachability Plot (eps = %.2f)' % eps,
                        legend_title_text='Cluster')
